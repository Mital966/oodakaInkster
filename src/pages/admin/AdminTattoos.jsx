...
function AdminTattoos() {
  const { tattoos, artistName, togglePublish, toggleFeatured, deleteTattoo } = useAdminData()
  ...
  async function handleToggle(fn, t, okMsg) {
    if (busyId) return
    setBusyId(t.id)
    try {
      await fn(t.id)
  ...
  async function handleDelete() {
    if (!toDelete) return
    try {
      await deleteTattoo(toDelete.id)
      toast('success', `"${toDelete.title}" was deleted.`)
      setToDelete(null)
    } catch (err) {
      toast('error', err.message || 'We couldn’t delete that tattoo. Please try again.')
      setToDelete(null)
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {filtered.length === 0 ? (
        <EmptyState
          title={tattoos.length === 0 ? 'No tattoos yet' : 'Nothing matches your search'}
          message={
            tattoos.length === 0
              ? 'Add your first piece and it will appear here, ready to publish to the site.'
              : 'Try a different title, artist or style.'
          }
          action={
            tattoos.length === 0 ? (
              <Link
                to="/admin/tattoos/new"
                className="inline-flex items-center gap-2 rounded-md bg-neutral-900 px-4 py-2 text-sm font-semibold text-neutral-100 transition-colors hover:bg-neutral-800"
              >
                <Plus size={15} /> Add your first tattoo
              </Link>
            ) : null
          }
        />
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-neutral-100">
                {filtered.map((t) => (
                  <tr key={t.id} className={cn('transition-colors hover:bg-neutral-50', busyId === t.id && 'opacity-50')}>
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <span className="block text-xs text-neutral-400">{t.category || 'Uncategorised'}</span>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-3 text-center">
                      <button
                        type="button"
                        onClick={() => handleToggle(togglePublish, t, t.published ? 'Moved to drafts.' : `${t.title} is now live on the site.`)}
                    ...
                    <td className="px-5 py-3 text-center">
                      <button
                        type="button"
                        onClick={() => handleToggle(toggleFeatured, t, t.featured ? 'Removed from homepage.' : `${t.title} now shows on the homepage.`)}
      ...
      <ConfirmModal
        open={!!toDelete}
        title="Delete tattoo?"
        message={toDelete ? `"${toDelete.title}" will be removed from the site and the admin library. This can't be undone.` : ''}
